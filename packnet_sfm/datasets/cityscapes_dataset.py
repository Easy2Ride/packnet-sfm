...
Kcity=np.array([[2262.52,   0.0               ,   1096.98],
...
def n_read_files(directory, ext=('.png', '.jpg', '.jpeg'), skip_empty=True):
    files = defaultdict(list)
...
class CityscapesDataset(Dataset):
    def __init__(self, root_dir, split, data_transform=None,
        ...
        self.files = []
        file_tree = n_read_files(root_dir)
        print(len(file_tree), "aaa", "\n\n\n\n\n\n")
    ...
    def __getitem__(self, idx):
        ...
        # intrinsics
        if "cityscapes" in self.root_dir.lower():
            intrinsics=Kcity
        else:
